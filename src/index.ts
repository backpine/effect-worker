 ...
 * object with a `fetch` handler that processes incoming HTTP and RPC requests.
 *
 ...
 * ### RPC
 * ```
 * /rpc → RPC (RpcGroup + RpcMiddleware)
 *   └─> Type-safe RPC procedures
 * ```
 *
 * ## Middleware-Based Request-Scoped Services
 *
 * Both HTTP and RPC use the same FiberRef bridge pattern:
 * - CloudflareBindingsMiddleware/RpcCloudflareMiddleware: Provides env/ctx
 * - DatabaseMiddleware/RpcDatabaseMiddleware: Creates per-request DB connection
 *
...
import { rpcRuntime, handleRpcRequest } from "@/rpc"
 ...
 *
 * This is the entry point for all HTTP and RPC requests to the Worker.
 */
export default {
